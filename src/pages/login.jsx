...
const Login = ({
  ...
  const input_data_structure = {
    role: {
      ...
      list_items: [
        ...
        { key: 5, id: "super-admin", title: "super Admin" },
      ],
    },
    email: {
      key: "email",
  ...
  const handle_submit = async () => {
    ...
    if (inputs.role.value === "manager") {
      ...
      staffLogin(data);
    } else if (inputs.role.value === "super-admin") {
      // Hardcoded credentials
      const hardcodedUsername = "admin@gmail.com";
      const hardcodedPassword = "1qaz2wsx";

      // Check if the entered credentials match the hardcoded ones
      if (
        inputs.email.value === hardcodedUsername &&
        inputs.password.value === hardcodedPassword
      ) {
        ...
        toast.error("Invalid username or password for Super Admin!");
      }
    }

    localStorage.setItem("role", inputs.role.value);
      ...
      <div className="grid grid-cols-12 bg-[#0234E2] min-h-[100vh]">
        ...
        <div className="col-span-6 bg-[#0131B7] rounded-l-[50px]">
          <div className="mt-[200px] ml-20 w-[80%]">
            <h1 className="text-white font-extrabold text-3xl">Login</h1>
            <p className="text-white">
              Select your role and Enter Email, Password
            </p>
            <Select input={inputs.role} handle_selection={handle_selection} />
